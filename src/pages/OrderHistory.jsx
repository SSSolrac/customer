        ...
        {orders.map((order) => {
          ...
          return (
            <article key={order.id} className="history-card">
              ...
              <p>
                {order.orderTypeLabel} • {order.paymentMethodLabel} • {order.items.length} items • <strong>{formatMoney(order.totalAmount)}</strong>
              </p>
              {(() => {
              ...
              <p className="history-items-summary">
                {order.items.slice(0, 2).map((item) => `${item.itemName} × ${item.quantity}`).join(", ")}
